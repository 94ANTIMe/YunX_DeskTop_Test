...
- **2026-09-22 · 夸克下载卡死修复（引擎侧）**：夸克任务 addUri 带 `lowest-speed-limit=1024`（≤1KB/s 约 30s 中止，僵死任务不再永久占并发槽）；`download_task` 新增 `fetch_ctx_json` 列（ALTER 迁移 + 旧任务空值回退旧直链），夸克取链三条路线（转存/直取/个人文件）写入 `{"fid":...}` 上下文，恢复/失败重试/重启重挂前按它刷新 `__puus` 并重新取直链（`resolve::refresh_quark_download_link`，失败回退旧链只记日志）；poll_loop 批量查态检测「活跃 gid 全失联」后台触发重挂（`remount_if_detached`，10s 冷却 + 引擎健康复检，不在 respawn_engine 原地重挂以免与 rpc_call 重试 addUri 双入队）；`resume` 不再无条件强写「下载中」，unpause/重入队后查 aria2 真实状态落库；`quark::poll_task` 连续 3 次非 200 带错误码失败（不再静默吞满 60s）；夸克登录提示区分「刚登录未落库」竞态。IPC：`DownloadLink` 增 `fetchCtx?`（skip_serializing_if 空串）、`enqueue_download` 增 `fetchCtx?` 参数——成对修改，旧前端不传仍兼容。触点：`aria2.rs`/`resolve.rs`/`api/quark.rs`/`api/pan_files.rs`/`models.rs`/`db/schema.rs`/`db/mod.rs`/`commands/download.rs`/`subscription.rs`/`ipc.ts`/`lib/download.ts`/`ResolvePage.tsx`/`PanFileManager.tsx`。兼容性：`download_task` 仅加列带默认值，旧库无感。

- **2026-09-22 · 前端界面整体升级（全局感知层 / 基础组件层 / 交互升级 / 清债）**：纯前端，IPC 与 Rust 零变化。新增 `lib/toast.ts`+`ToastHost`（全局 Toast，aria-live）、`hooks/useDownloads.ts`（下载任务单例 store：事件合并/速度采样/终态迁移自动 toast/派生统计，INERT 哨兵保住隐藏页零重渲染）、`components/ui/`（Button/IconButton/Toggle/Select/Skeleton/SliderRow/Modal/Drawer/ConfirmDialog + 单测）；顶栏下载 pill 加任务数角标与速度；全部弹层迁入 ui/Modal|Drawer（删旧 ConfirmDialog，净 -212 行）；交互新增解析多选批量下载（底部操作条 + batchBusy 独立守卫 + 单行互斥放宽）、破坏性确认 ×8、Ctrl+1~7 / Ctrl+V 快捷键、DrivePage 下拉点击外部/Esc 关闭、StatsPage 柱状图键盘可达、Onboarding 三步流程修复、骨架屏 ×4、LogsPage 行 memo；SettingsPage 5 滑杆/3 下拉/2 开关收编为 ui 原语，全库手写 select/range/switch 清零。新约定见 ui-and-themes.md「弹层、Toast 与 z 层级」。兼容性：存量 36 例测试语义保持（两处断言随弹层挂载 ToastHost），`maxConcurrentDownloads` 等设置项与数据格式零变化。执行蓝本：ui-upgrade-plan.md。
...
- **影响**：落点 `src-tauri/src/{credentials.rs,resolve.rs,aria2/,models/,api/}`；architecture.md 分层规则随 C2/C4/C5/C6 各步落地后增量修订；cli.rs、subscription.rs 等第三方调用方随 C4 收敛；对旧数据与 IPC 无兼容影响。
- **执行结果（2026-09-22）**：C1–C6 Rust 侧与 C7 状态注册表全部落地；**ipc.ts 分域与 ResolvePage 区块深拆经评估暂缓**——前者 497 行且分节清晰、与 models.rs 的成对修改约定不受影响，拆分属纯文本搬运无去重收益；后者需先补表征测试再动（当前仅 2 例页面测试），在测试补齐前不拆。两项留待后续按需立项，不阻塞本 ADR 验收。
