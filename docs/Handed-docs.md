...
- 2026-09-22 11:45 排障定案：下载反复失败真因 = CDN 走 IPv6 无路由（curl -4 通/-6 死），aria2 不回退 IPv4；引擎 spawn 加 `--disable-ipv6=true`（decisions.md 同日变更记录），重打包装机（主程序 21239296 字节、时间戳 11:44）。用户双击 `yunx-desktop.exe` 后启动恢复应能正常续传，待确认。
