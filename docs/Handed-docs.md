...
- 2026-09-22 12:20 真机结论 + 重挂误判修复：**ELDEN RING.part01.exe（3.9GB）已完整下载到 Downloads**（mtime 11:59，与引擎 completedLength 分毫不差——IPv6 修复生效，夸克下载在本机正式跑通）。用户截图的「失联恢复」每 10 秒刷屏 = 大文件满速下载期引擎 RPC 偶发超时 → 批量查询整批失败 → 误判失联；且重挂的 live 复检也一并超时导致 7 次重复入队（同文件覆盖写，最终完整无害）。修复 `01b0c71` 之后：单轮失败不计、连续 3 轮全失联才重挂 + 重挂前逐任务 tellStatus 复核。重装校验通过。：下载反复失败真因 = CDN 走 IPv6 无路由（curl -4 通/-6 死），aria2 不回退 IPv4；引擎 spawn 加 `--disable-ipv6=true`（decisions.md 同日变更记录），重打包装机（主程序 21239296 字节、时间戳 11:44）。用户双击 `yunx-desktop.exe` 后启动恢复应能正常续传，待确认。
- 2026-09-22 11:03 修复登出按钮存量缺陷（`invalid args key`：TS 侧 key? 传 undefined 被 Tauri 丢弃，Rust 侧必填 String；现缺省传空串命中「登出当前账号」分支，全库 34 个命令审计仅此一处）并重装：D:\YunX 主程序时间戳 11:01，校验通过。用户侧另见 `capacity limit` 转存失败 = 夸克盘容量满（多实例重复转存所致），已指引清理「YunX临时转存」目录。
